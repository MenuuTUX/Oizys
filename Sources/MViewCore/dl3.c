...
static const MViewDL3Profile RIDGE_6000 = {
    .product_id = 0x6000,
    .head_count = 2,
...
static size_t push_frame(uint8_t *out, size_t cap, uint32_t type, uint16_t sub, uint16_t aux,
    ...
    mview_dl3_header(out, type, sub, aux, seq, body_len);
    if (body_len) {
...
size_t mview_dl3_seal_cp(uint8_t *out, size_t cap, const uint8_t ks[16], const uint8_t riv[8],
                         ...
                         size_t inner_len) {
    uint8_t key[16];
    mview_cp_session_key(ks, key);
    return mview_dl3_seal_live(out, cap, key, riv, 0x24, aux_for_id(inner_id), wire_seq, inner,
                               inner_len);
}
...
size_t mview_dl3_seal_live(uint8_t *out, size_t cap, const uint8_t live_key[16],
    ...
    mview_dl3_header(out, 4, wire_sub, aux, wire_seq, body_len);
    uint8_t *ct = out + 16;
    mview_aes_ctr_xor(live_key, riv, wire_seq, plain, ct, plain_len);
    uint8_t mac_nonce[8];
    ...
    mview_aes_cmac(live_key, macbuf, 16 + plain_len, ct + plain_len);
    return total;
}
...
int mview_dl3_open_cp(const uint8_t ks[16], const uint8_t in_riv[8], uint32_t seq,
    ...
    uint8_t tag[16];
    mview_aes_cmac(key, macbuf, 16 + ct_len, tag);
    uint8_t diff = 0;
    for (int i = 0; i < 16; i++) {
    ...
    mview_aes_ctr_xor(key, in_riv, seq, body, pt, ct_len);
    return (int)ct_len;
}
