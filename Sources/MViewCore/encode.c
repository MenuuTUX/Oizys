...
void mview_damage_init(MViewDamageMap *map, uint32_t width, uint32_t height) {
    ...
    map->width = width;
    map->height = height;
    map->cols = (width + MVIEW_STRIP_W - 1) / MVIEW_STRIP_W;
    map->rows = (height + MVIEW_STRIP_H - 1) / MVIEW_STRIP_H;
...
MViewStrip mview_damage_geom(const MViewDamageMap *map, uint32_t col, uint32_t row) {
    ...
    s.col = col;
    s.row = row;
    s.x = col * MVIEW_STRIP_W;
    s.y = row * MVIEW_STRIP_H;
    s.w = MVIEW_STRIP_W;
    s.h = MVIEW_STRIP_H;
...
#define MVIEW_HASH_PRIME 0x9e3779b185ebca87ull
 ...
 */
static uint64_t hash_strip(const uint8_t *bgra, size_t stride, MViewStrip s) {
...
int mview_damage_update(MViewDamageMap *map, const uint8_t *bgra, size_t stride, MViewStrip *out,
                        int max_out) {
    int n = 0;
    for (uint32_t row = 0; row < map->rows; row++) {
        for (uint32_t col = 0; col < map->cols; col++) {
            MViewStrip s = mview_damage_geom(map, col, row);
...
static void charge_macro_tile(MViewDamageMap *map, uint32_t col, uint32_t row) {
    uint32_t first_col = col - col % MVIEW_MACRO_STRIPS;
    uint32_t first_row = row - row % MVIEW_MACRO_STRIPS;
    for (uint32_t r = first_row; r < first_row + MVIEW_MACRO_STRIPS && r < map->rows; r++) {
        for (uint32_t c = first_col; c < first_col + MVIEW_MACRO_STRIPS && c < map->cols; c++) {
            map->debt[r * map->cols + c] = MVIEW_DAMAGE_REPEATS;
        }
    }
}
...
int mview_damage_owed(const MViewDamageMap *map, MViewStrip *out, int max_out) {
    ...
    for (uint32_t row = 0; row < map->rows; row++) {
        for (uint32_t col = 0; col < map->cols; col++) {
            ...
            if (n < max_out) {
                out[n] = mview_damage_geom(map, col, row);
            }
            n++;
        }
    }
...
static int settle_plan(MViewDamageMap *map, MViewStrip *out, int max_out, int *presentations) {
    ...
    /* A keyframe has to reach every dock buffer at once: it clears the ledger on its own
       ...
       MVIEW_DOCK_BUFFERS times; at that count the third slot kept the black training
