...
const char *const kMViewZoneNames[MVIEW_ZONE_COUNT] = {
...
typedef struct ThreadStats {
    ZoneStats zones[MVIEW_ZONE_COUNT];
...
static _Thread_local MViewZone g_stack[32];
...
uint64_t mview_profile_push(MViewZone zone) {
    if (zone < MVIEW_ZONE_COUNT && g_depth < (int)(sizeof(g_stack) / sizeof(g_stack[0]))) {
...
static void merge(ZoneStats *out) {
    memset(out, 0, sizeof(*out) * MVIEW_ZONE_COUNT);
    for (ThreadStats *t = atomic_load(&g_threads); t; t = t->next) {
        for (int i = 0; i < MVIEW_ZONE_COUNT; i++) {
...
double mview_profile_total_ms(MViewZone zone) {
    if (zone >= MVIEW_ZONE_COUNT) {
        return 0.0;
    }
    ZoneStats totals[MVIEW_ZONE_COUNT];
    merge(totals);
...
const char *mview_profile_zone_name(MViewZone zone) {
    return zone < MVIEW_ZONE_COUNT ? kMViewZoneNames[zone] : "";
}

int mview_profile_zone_count(void) {
    return MVIEW_ZONE_COUNT;
}

void mview_profile_report(const char *title) {
    ZoneStats totals[MVIEW_ZONE_COUNT];
    merge(totals);
    double total = ticks_to_ms(totals[MVIEW_ZONE_PRESENT].ticks);
    if (total <= 0) {
        for (int i = 0; i < MVIEW_ZONE_COUNT; i++) {
