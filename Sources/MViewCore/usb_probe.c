...
MViewFamily mview_family_from_platform(const char *name) {
    if (name && strcmp(name, "RidgeDoc") == 0) {
        return MVIEW_FAMILY_RIDGE;
    }
    if (name && strcmp(name, "NavaDock") == 0) {
        return MVIEW_FAMILY_NAVARRO;
    }
    if (name && strcmp(name, "EllaDock") == 0) {
        return MVIEW_FAMILY_ELLA;
    }
    if (name && strcmp(name, "FflyMoni") == 0) {
        return MVIEW_FAMILY_FIREFLY;
    }
    return MVIEW_FAMILY_UNKNOWN;
}

const char *mview_family_name(MViewFamily family) {
    switch (family) {
    case MVIEW_FAMILY_RIDGE:
        return "Ridge (DL-6xxx)";
    case MVIEW_FAMILY_NAVARRO:
        return "Navarro (DL-7000)";
    case MVIEW_FAMILY_ELLA:
        return "Ella (DL-3/5xxx)";
    case MVIEW_FAMILY_FIREFLY:
...
int mview_identity_parse(MViewIdentity *id, const uint8_t *bytes, int n) {
    ...
    if (n < 16 || bytes[0] < 16 || bytes[1] != MVIEW_IDENT_TYPE) {
        return -1;
    }
    ...
    id->platform[8] = 0;
    id->family = mview_family_from_platform(id->platform);
    id->valid = 1;
    return 0;
}

static void add_endpoint(MViewInterface *iface, io_service_t pipe) {
    if (iface->ep_count >= MVIEW_MAX_EP) {
        return;
    }
    ...
    MViewEndpoint *e = &iface->ep[iface->ep_count++];
    e->address = a;
    e->dir_in = (a & 0x80) ? 1 : 0;
    e->transfer = at & 0x03;
...
static void inspect_interface(MViewHub *hub, io_service_t iface) {
    if (hub->iface_count >= MVIEW_MAX_IFACE) {
        return;
    }
    MViewInterface *in = &hub->iface[hub->iface_count];
    ...
    if (in->class_code == MVIEW_DL3_CLASS && in->protocol == MVIEW_DL3_PROTOCOL &&
        in->ep_count == 0) {
        static const MViewEndpoint kRidge[] = {
...
static int inspect_device(io_service_t svc, MViewHub *hub) {
...
void mview_hub_print(const MViewHub *hub) {
    ...
    if (hub->identity.valid) {
        printf("  identity   %s  family=%s\n", hub->identity.platform,
               mview_family_name(hub->identity.family));
    } else {
        printf("  identity   (type 0x40 not in registry; claim to read it)\n");
    }
    if (hub->identity.family == MVIEW_FAMILY_RIDGE || mview_hub_is_dl3(hub)) {
        printf("  profile    2 heads, video OUT 08/0b, strips 64x16 (Ridge)\n");
        printf("  panels     2 × 1920×1080@60 (Dell P2219H)\n");
        MViewDamageMap map;
        mview_damage_init(&map, 1920, 1080);
        printf("  strips     %u×%u  (%u tiles/head/frame)\n", map.cols, map.rows,
               map.cols * map.rows);
    }
    printf("  dl3        %s   control 0x02/0x84 %s\n", mview_hub_is_dl3(hub) ? "yes" : "no",
           mview_hub_has_control(hub) ? "present" : "MISSING");
    uint8_t vouts[8];
    int vn = mview_hub_video_outs(hub, vouts, 8);
    printf("  video OUT  [");
    ...
    if (hub->exclusive_owner[0]) {
        printf("  owner      %s\n", hub->exclusive_owner);
        printf("             probe is read-only; `mview run --takeover` claims the hub\n");
    } else {
        printf("  owner      none — USB is free to claim\n");
    }
    printf("  interfaces\n");
    for (int i = 0; i < hub->iface_count; i++) {
        const MViewInterface *in = &hub->iface[i];
        const char *tag = "";
        ...
        for (int e = 0; e < in->ep_count; e++) {
            const MViewEndpoint *ep = &in->ep[e];
...
int mview_hub_is_dl3(const MViewHub *hub) {
    for (int i = 0; i < hub->iface_count; i++) {
        const MViewInterface *in = &hub->iface[i];
        if (in->class_code == MVIEW_DL3_CLASS && in->subclass == MVIEW_DL3_SUBCLASS &&
            in->protocol == MVIEW_DL3_PROTOCOL) {
            return 1;
        }
    }
    return 0;
...
int mview_hub_has_control(const MViewHub *hub) {
    ...
    for (int i = 0; i < hub->iface_count; i++) {
        for (int e = 0; e < hub->iface[i].ep_count; e++) {
            const MViewEndpoint *ep = &hub->iface[i].ep[e];
            if (ep->address == MVIEW_EP_CTRL_OUT && !ep->dir_in) {
                out = 1;
            }
            if (ep->address == MVIEW_EP_CTRL_IN && ep->dir_in) {
                in = 1;
            }
        }
    }
...
int mview_hub_video_outs(const MViewHub *hub, uint8_t *out, int max_out) {
    ...
    for (int i = 0; i < hub->iface_count; i++) {
        ...
        for (int e = 0; e < in->ep_count; e++) {
            const MViewEndpoint *ep = &in->ep[e];
            if (!ep->dir_in && ep->transfer == 2 && ep->address != MVIEW_EP_CTRL_OUT) {
                if (n < max_out) {
                    out[n] = ep->address;
                }
                n++;
