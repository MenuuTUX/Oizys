...
static io_service_t unique_device(void) {
    ...
    while ((service = IOIteratorNext(iterator))) {
        if (number(service, CFSTR("idVendor")) == MVIEW_VID) {
            count++;
...
static void *user_client(io_service_t service, CFUUIDRef plugin_type, CFUUIDRef interface_type) {
    ...
    if (!result) mview_log("USB user client unavailable: 0x%08x", rc);
    return result;
}
...
static USBInterface **open_interface(MViewSession *s, uint8_t desired) {
    ...
    if (rc) { mview_log("open USB interface %u failed: 0x%08x", desired, rc); (*found)->Release(found); return NULL; }
...
static void *read_control(void *context) {
    MViewSession *s = context;
    uint8_t bytes[IN_BUF];
    while (!atomic_load(&s->closing)) {
        UInt32 length = sizeof(bytes);
        IOReturn rc = (*s->iface)->ReadPipeTO(s->iface, s->pipes[MVIEW_EP_CTRL_IN], bytes, &length, 250, 250);
...
MViewSession *mview_session_open(int capture) {
    ...
    if (rc) { mview_log("open USB device failed: 0x%08x", rc); goto fail; }
    ...
    s->reader_started = 1;
    mview_log("C USB session open: endpoints 02/84/08/0b, identity=%d bytes", s->identity_len);
    return s;
fail:
    mview_session_close(s);
    return NULL;
}

int mview_session_bulk_out(MViewSession *s, uint8_t ep, const void *data, size_t length) {
    ...
    if (rc) { mview_log("bulk OUT 0x%02x %zu failed: 0x%08x", ep, length, rc); return -1; }
    return (int)length;
}
int mview_session_bulk_out_frame(MViewSession *s, uint8_t ep, const void *data, size_t length) {
    return mview_session_bulk_out(s, ep, data, length);
}
int mview_session_bulk_outv(MViewSession *s, uint8_t ep, const MViewUSBChunk *chunks, int count) {
    if (!chunks || count <= 0) return -1;
    size_t total = 0;
    for (int i = 0; i < count; i++) {
        if (chunks[i].length > INT_MAX - total) return -1;
        int written = mview_session_bulk_out(s, ep, chunks[i].bytes, chunks[i].length);
        if (written < 0) return -1;
        total += (size_t)written;
    }
    return (int)total;
}
int mview_session_recv(MViewSession *s, void *data, size_t capacity, size_t *got, double timeout) {
    if (got) *got = 0;
...
int mview_session_ctrl(MViewSession *s, uint8_t bm, uint8_t request, uint16_t value, uint16_t index,
    ...
    if (rc || r.wLenDone > length) { mview_log("USB control %02x/%02x failed: 0x%08x", bm, request, rc); return -1; }
    return (int)r.wLenDone;
}
int mview_session_get_identity(MViewSession *s, uint8_t *bytes, int capacity) {
...
void mview_session_close(MViewSession *s) {
    ...
    if (s->reader_started) {
        (*s->iface)->AbortPipe(s->iface, s->pipes[MVIEW_EP_CTRL_IN]);
