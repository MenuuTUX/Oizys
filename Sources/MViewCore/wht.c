 ...
 */
#ifndef MVIEW_CAP_SYNC
#define MVIEW_CAP_SYNC 7
#endif
#ifndef MVIEW_CAP_DC
#define MVIEW_CAP_DC 10
#endif
#ifndef MVIEW_CAP_AC
#define MVIEW_CAP_AC 10
#endif
/*
 ...
 */
#ifndef MVIEW_CAP_AC_LUMA
#define MVIEW_CAP_AC_LUMA 9
#endif
...
size_t mview_video_colour_strip_planes(uint8_t *out, size_t capacity, uint16_t x, uint16_t y,
    ...
    for (unsigned block = 0; block < 16; block++) {
        for (unsigned plane = 0; plane < 3; plane++) {
            ...
            MVIEW_PROFILE_BEGIN(haar, MVIEW_ZONE_HAAR);
            haar_pyramid(transformed);
            MVIEW_PROFILE_END(haar, MVIEW_ZONE_HAAR);
            MVIEW_PROFILE_BEGIN(quant, MVIEW_ZONE_QUANTIZE);
            ...
            MVIEW_PROFILE_END(quant, MVIEW_ZONE_QUANTIZE);
        }
    }

    MVIEW_PROFILE_BEGIN(entropy, MVIEW_ZONE_ENTROPY);
    ...
    for (unsigned block = 0; block < 16; block++) {
        for (unsigned plane = 0; plane < 3; plane++) {
            ...
            ridge_symbol(&main, value, MVIEW_CAP_SYNC);
        }
    }
    ...
    for (unsigned block = 0; block < 16; block++) {
        for (unsigned plane = 0; plane < 3; plane++) {
            ...
            ridge_symbol(&main, dc - previous[plane], MVIEW_CAP_DC);
            previous[plane] = dc;
        }
    }
    ...
    for (unsigned block = 0; block < 16; block++) {
        ...
        for (unsigned plane = 0; plane < 3; plane++) {
            ...
            unsigned cap = plane == 2 ? MVIEW_CAP_AC_LUMA : MVIEW_CAP_AC;
            for (unsigned position = 0; position < 63; position++) {
    ...
    MVIEW_PROFILE_END(entropy, MVIEW_ZONE_ENTROPY);
    return length;
}
...
size_t mview_video_colour_strip_bgra(uint8_t *out, size_t capacity, uint16_t x, uint16_t y,
                                     const uint8_t *bgra, size_t stride, uint32_t width,
                                     uint32_t height) {
    if (!bgra || stride < (size_t)width * 4) {
        return 0;
    }
    MVIEW_PROFILE_BEGIN(strip, MVIEW_ZONE_STRIP);
    MVIEW_PROFILE_BEGIN(convert, MVIEW_ZONE_CONVERT);
    int32_t planes[16][3][64];
    if ((size_t)x + MVIEW_STRIP_W <= width && (size_t)y + MVIEW_STRIP_H <= height) {
    ...
    MVIEW_PROFILE_END(convert, MVIEW_ZONE_CONVERT);
    size_t encoded = mview_video_colour_strip_planes(out, capacity, x, y, &planes[0][0][0]);
    MVIEW_PROFILE_END(strip, MVIEW_ZONE_STRIP);
    return encoded;
}
