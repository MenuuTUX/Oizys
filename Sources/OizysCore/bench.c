...
int oizys_bench_encoder(void) {
    ...
    OizysStrip *owed = calloc(OIZYS_MAX_STRIPS, sizeof(*owed));
    uint8_t *bodies = malloc((size_t)OIZYS_MAX_STRIPS * BENCH_BODY);
    ...
    for (int round = 0; round < rounds; round++) {
        oizys_damage_init(map, BENCH_W, BENCH_H);
        double t = now_ms();
        keyframe_strips =
        ...
        for (uint32_t i = 0; i < strips; i++) {
            oizys_video_colour_strip_bgra(bodies + (size_t)i * BENCH_BODY, BENCH_BODY,
                                          (uint16_t)((i % cols) * OIZYS_STRIP_W),
                                          (uint16_t)((i / cols) * OIZYS_STRIP_H), bgra, stride,
                                          BENCH_W, BENCH_H);
        }
        ...
        dispatch_apply(strips, DISPATCH_APPLY_AUTO, ^(size_t i) {
          oizys_video_colour_strip_bgra(bodies + i * BENCH_BODY, BENCH_BODY,
                                        (uint16_t)((i % cols) * OIZYS_STRIP_W),
                                        (uint16_t)((i / cols) * OIZYS_STRIP_H), bgra, stride,
                                        BENCH_W, BENCH_H);
        });
        ...
        for (int repeat = 0; repeat < 200; repeat++) {
            for (uint32_t k = 0; k < macro; k++) {
                oizys_video_colour_strip_bgra(bodies + (size_t)k * BENCH_BODY, BENCH_BODY,
                                              (uint16_t)((k % OIZYS_MACRO_STRIPS) * OIZYS_STRIP_W),
                                              (uint16_t)((k / OIZYS_MACRO_STRIPS) * OIZYS_STRIP_H),
...
int oizys_profile_encoder(void) {
    ...
    for (int frame = 0; frame < frames; frame++) {
        ...
        int presentations = 0;
        int count = oizys_damage_plan(map, bgra, stride, owed, OIZYS_MAX_STRIPS, &presentations);
        total_strips += count;
        ...
        oizys_profile_record(OIZYS_ZONE_ENCODE_STRIPS, encode_start, mach_absolute_time());
        oizys_damage_presented(map);
        oizys_profile_record(OIZYS_ZONE_PRESENT, frame_start, mach_absolute_time());
    }
    oizys_profile_enable(0);
    ...
    oizys_profile_report("scanout profile");
    printf("\nper frame: %.3f ms wall, budget at 60 Hz is %.2f ms\n",
           oizys_profile_total_ms(OIZYS_ZONE_PRESENT) / frames, 1000.0 / 60);
    (void)cols;
    free(bgra);
    free(map);
    free(owed);
