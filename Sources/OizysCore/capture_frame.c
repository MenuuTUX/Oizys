...
OizysCaptureOutput *oizys_output_create(OizysDriver *driver, uint8_t head, dispatch_queue_t queue) {
    OizysCaptureOutput *o = calloc(1, sizeof(*o));
    if (!o) return NULL;
...
void oizys_output_disable(OizysCaptureOutput *o) {
...
void oizys_output_fail(OizysCaptureOutput *o, const char *message) {
    os_unfair_lock_lock(&o->lock);
...
static void present(OizysCaptureOutput *o, CMSampleBufferRef sample, uint64_t displayed,
    ...
    if (CVPixelBufferLockBaseAddress(pixels, kCVPixelBufferLock_ReadOnly) != kCVReturnSuccess) {
        oizys_output_fail(o, "could not lock captured pixels");
        return;
    }
    ...
    if ((frame == 0 || frame == 30) && OIZYS_DIAGNOSTICS && oizys_config()->capture_dump_frames) {
        char path[128];
    ...
    if (OIZYS_DIAGNOSTICS && frame % 60 == 0 && strcmp(oizys_config()->log_level, "debug") == 0) {
    ...
    int result = oizys_driver_present_bgra_dirty(o->driver, o->head, base, stride, width, height,
                                                rects, rect_count);
    if (result < 0 && OIZYS_DIAGNOSTICS && oizys_config()->capture_dump_frames) {
        char path[96];
    ...
    CVPixelBufferUnlockBaseAddress(pixels, kCVPixelBufferLock_ReadOnly);
    if (result < 0) {
        oizys_output_fail(o, "desktop presentation failed; see logs/run.log");
    } else {
...
static void consume(OizysCaptureOutput *o) {
    OizysDirtyRect rects[OIZYS_CAPTURE_MAX_RECTS];
    os_unfair_lock_lock(&o->lock);
...
void oizys_output_enqueue(OizysCaptureOutput *o, CMSampleBufferRef sample, int status,
                          uint64_t timestamp, const OizysDirtyRect *rects, int rect_count) {
    if (!o || status != 0 || !sample || !CMSampleBufferIsValid(sample)) return;
    if (!rects || rect_count < 0 || rect_count > OIZYS_CAPTURE_MAX_RECTS) rect_count = 0;
    os_unfair_lock_lock(&o->lock);
    ...
    if (rect_count == 0 || o->pending_rect_count + rect_count > OIZYS_CAPTURE_MAX_RECTS) {
...
int oizys_output_needs_refresh(OizysCaptureOutput *o) {
    ...
    return elapsed >= 2.0 * NSEC_PER_SEC / oizys_config()->capture_fps;
}

int oizys_output_frames(const OizysCaptureOutput *o) { return o ? atomic_load(&o->frames) : 0; }
const char *oizys_output_failure(const OizysCaptureOutput *o) { return o && atomic_load(&o->failed) ? o->failure : NULL; }
void oizys_output_destroy(OizysCaptureOutput *o) { if (o) { oizys_output_disable(o); free(o); } }

void oizys_output_report(OizysCaptureOutput *o) {
#if OIZYS_DIAGNOSTICS
    mach_timebase_info_data_t tb; mach_timebase_info(&tb);
