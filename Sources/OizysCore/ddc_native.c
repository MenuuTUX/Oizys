...
static int load_ioav(void) {
    ...
    if (!av_create || !av_read || !av_write || !av_edid) {
        oizys_log("IOAVService is not available in this IOKit; DDC/CI is off");
        return 0;
    }
    return 1;
}
 ...
 */
static IOReturn ddc_write_framed(OizysDDCDisplay *display, int framing, const uint8_t *payload,
                                 size_t payload_len) {
    uint8_t frame[40];
...
static int ddc_exchange(OizysDDCDisplay *display, const uint8_t *payload, size_t payload_len,
    ...
    if (CFDataGetLength(edid) >= 128) {
        ...
        if (memcmp(bytes, header, sizeof(header)) == 0 && (sum & 255) == 0) {
            ...
            for (uint32_t i = 0; i < count; i++) {
                if (CGDisplayIsBuiltin(ids[i]) || oizys_display_is_sidecar(ids[i]) ||
...
OizysDDCDisplay *oizys_ddc_open(uint32_t display_id) {
    ...
    OizysDDCDisplay *display = (OizysDDCDisplay *)calloc(1, sizeof(*display));
    if (!display) {
        CFRelease(av);
        return NULL;
    }
...
int oizys_ddc_set_vcp(OizysDDCDisplay *display, uint8_t code, uint16_t value) {
    ...
    if (display->framing < 0) {
        ...
        oizys_ddc_get_vcp(display, OIZYS_VCP_VERSION, NULL, NULL);
...
const char *oizys_ddc_vcp_name(uint8_t code) {
    switch (code) {
    case OIZYS_VCP_RESTORE_FACTORY: return "restore factory defaults";
    case OIZYS_VCP_RESTORE_LUMINANCE: return "restore brightness and contrast";
    case OIZYS_VCP_RESTORE_COLOUR: return "restore colour defaults";
    case OIZYS_VCP_LUMINANCE: return "brightness";
    case OIZYS_VCP_CONTRAST: return "contrast";
    case OIZYS_VCP_COLOUR_PRESET: return "colour preset";
    case OIZYS_VCP_GAIN_RED: return "video gain red";
    case OIZYS_VCP_GAIN_GREEN: return "video gain green";
    case OIZYS_VCP_GAIN_BLUE: return "video gain blue";
    case OIZYS_VCP_AUTO_SETUP: return "auto setup";
    case OIZYS_VCP_INPUT_SOURCE: return "input source";
    case OIZYS_VCP_AUDIO_VOLUME: return "speaker volume";
    case OIZYS_VCP_BLACK_RED: return "black level red";
    case OIZYS_VCP_BLACK_GREEN: return "black level green";
    case OIZYS_VCP_BLACK_BLUE: return "black level blue";
    case OIZYS_VCP_AUDIO_MUTE: return "audio mute / screen blank";
    case OIZYS_VCP_USAGE_HOURS: return "display usage hours";
    case OIZYS_VCP_CONTROLLER_TYPE: return "controller type";
    case OIZYS_VCP_FIRMWARE: return "firmware level";
    case OIZYS_VCP_OSD_LOCK: return "OSD button lock";
    case OIZYS_VCP_OSD_LANGUAGE: return "OSD language";
    case OIZYS_VCP_POWER_MODE: return "power mode";
    case OIZYS_VCP_VERSION: return "MCCS version";
    default: return NULL;
    }
}

static const uint8_t STANDARD_CODES[] = {
    OIZYS_VCP_LUMINANCE,    OIZYS_VCP_CONTRAST,    OIZYS_VCP_COLOUR_PRESET,
    OIZYS_VCP_GAIN_RED,     OIZYS_VCP_GAIN_GREEN,  OIZYS_VCP_GAIN_BLUE,
    OIZYS_VCP_BLACK_RED,    OIZYS_VCP_BLACK_GREEN, OIZYS_VCP_BLACK_BLUE,
    OIZYS_VCP_INPUT_SOURCE, OIZYS_VCP_AUDIO_VOLUME, OIZYS_VCP_AUDIO_MUTE,
    OIZYS_VCP_OSD_LOCK,     OIZYS_VCP_OSD_LANGUAGE, OIZYS_VCP_POWER_MODE,
    OIZYS_VCP_USAGE_HOURS,  OIZYS_VCP_CONTROLLER_TYPE, OIZYS_VCP_FIRMWARE,
    OIZYS_VCP_VERSION,
};
...
void oizys_ddc_dump(OizysDDCDisplay *display, FILE *out) {
    for (size_t i = 0; i < sizeof(STANDARD_CODES); i++) {
        ...
        if (oizys_ddc_get_vcp(display, code, &current, &maximum) != 0) {
            continue;
        }
        fprintf(out, "  0x%02x  %-32s %5u / %u\n", code, oizys_ddc_vcp_name(code), current,
                maximum);
    }
}
...
void oizys_ddc_list(FILE *out) {
    ...
    int available = 0;
    for (uint32_t i = 0; i < count; i++) {
        OizysDDCDisplay *display = oizys_ddc_open(ids[i]);
        available += display != NULL;
        const char *kind = CGDisplayIsBuiltin(ids[i])          ? "built-in"
                           : oizys_display_is_sidecar(ids[i])  ? "Sidecar/AirPlay"
                           : CGDisplayVendorNumber(ids[i]) == 0x4d56 ? "Oizys virtual head"
                                                                     : "external";
    ...
    if (!available) {
        fputs("\n  No native I2C path could be verified. This backend requires an external\n"
              ...
              "  separate USB DDC/CI tunnel, which Oizys does not yet implement.\n", out);
    }
}
