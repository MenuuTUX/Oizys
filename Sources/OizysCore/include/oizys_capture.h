#ifndef OIZYS_CAPTURE_H
#define OIZYS_CAPTURE_H
...
typedef struct OizysCapture OizysCapture;
 ...
 */
OizysCapture *oizys_capture_start(const uint32_t *display_ids, int count, OizysDriver *driver,
 ...
 */
void oizys_capture_start_refresh_clock(OizysCapture *capture, OizysDriver *driver, int heads,
                                       int hz);

int oizys_capture_frames(const OizysCapture *capture, int head);
/* NULL while the head is healthy. */
const char *oizys_capture_failure(const OizysCapture *capture, int head);
int oizys_capture_head_count(const OizysCapture *capture);
/* Snapshot/reset profiler counters only after capture and encoder work have quiesced. */
void oizys_capture_profile_report(OizysCapture *capture, const char *title);
void oizys_capture_stop(OizysCapture *capture);
