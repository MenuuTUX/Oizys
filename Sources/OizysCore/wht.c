...
/*
 * Eight bytes through a 256-entry table.
 *
 * vqtbl4q_u8 looks up 64 entries and yields zero for any index at or above 64, so four of
 * them cover the range: each one is offset by 64 and the wrap-around puts every index
 * outside its own quarter above 64, where it reads as zero. Exactly one quarter answers for
 * any index, so an OR combines them without a select.
 */
static inline uint8x8_t table_lookup(const uint8x16x4_t *table, uint8x8_t index) {
    uint8x16_t wide = vcombine_u8(index, index);
    uint8x16_t out = vqtbl4q_u8(table[0], wide);
    out = vorrq_u8(out, vqtbl4q_u8(table[1], vsubq_u8(wide, vdupq_n_u8(64))));
    out = vorrq_u8(out, vqtbl4q_u8(table[2], vsubq_u8(wide, vdupq_n_u8(128))));
    out = vorrq_u8(out, vqtbl4q_u8(table[3], vsubq_u8(wide, vdupq_n_u8(192))));
    return vget_low_u8(out);
}

static inline void load_table(uint8x16x4_t *out, const uint8_t *source) {
...
static inline void convert_pixel_row(const uint8_t *pixels, int32_t *blue_plane,
                                     ...
                                     const uint8x16x4_t *tables) {
    uint8x8x4_t bgra = vld4_u8(pixels);
    if (tables) {
        /* vld4 has already separated the channels, so this is the cheapest point in the
         * frame at which a per-channel table can be applied. */
        bgra.val[0] = table_lookup(tables + 8, bgra.val[0]);   /* blue  */
        bgra.val[1] = table_lookup(tables + 4, bgra.val[1]);   /* green */
        bgra.val[2] = table_lookup(tables + 0, bgra.val[2]);   /* red   */
    }
        ...
        for (unsigned block = 0; block < 16; block++) {
            ...
            for (unsigned py = 0; py < 8; py++) {
                convert_pixel_row(rows + (size_t)py * stride, &planes[block][0][py * 8],
                                  &planes[block][1][py * 8], &planes[block][2][py * 8], active);
            }
        }
    } else {
