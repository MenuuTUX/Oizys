...
private final class CaptureDelegate: NSObject, SCStreamOutput, SCStreamDelegate {
    ...
    init?(_ driver: OpaquePointer, _ head: UInt8, _ queue: DispatchQueue) {
        guard let output = oizys_output_create(driver, head, queue) else { return nil }
        self.output = output
    }
    deinit { oizys_output_destroy(output) }
    func stream(_ stream: SCStream, didOutputSampleBuffer sampleBuffer: CMSampleBuffer, of type: SCStreamOutputType) {
        ...
        rects.withUnsafeBufferPointer {
            oizys_output_enqueue(output, sampleBuffer, status.int32Value, displayed,
                                 $0.baseAddress, Int32($0.count))
        }
    }
    func stream(_ stream: SCStream, didStopWithError error: Error) {
        "ScreenCaptureKit stopped: \(error.localizedDescription)".withCString { oizys_output_fail(output, $0) }
    }
}
...
private func dirtyRects(_ info: [SCStreamFrameInfo: Any]) -> [OizysDirtyRect] {
    guard let raw = info[.dirtyRects] as? [[String: Any]], !raw.isEmpty,
          raw.count <= Int(OIZYS_CAPTURE_MAX_RECTS) else { return [] }
    var out: [OizysDirtyRect] = []
...
private final class Capture {
    let count: Int
    let worker = DispatchQueue(label: "org.oizys.capture.serial", qos: .userInteractive)
    let ingest = DispatchQueue(label: "org.oizys.capture.ingest", qos: .userInteractive)
    var streams: [SCStream] = []
    var outputs: [Int: CaptureDelegate] = [:]
    #if !OIZYS_PRODUCTION
    var displayIDs: [Int: UInt32] = [:]
    var previousFrames: [Int: Int32] = [:]
...
@_cdecl("oizys_capture_start_refresh_clock")
func captureClock(_ raw: UnsafeMutableRawPointer?, _ driver: OpaquePointer?, _ heads: Int32, _ hz: Int32) {
    ...
    timer.setEventHandler { [weak capture] in
        guard let capture, capture.outputs.values.allSatisfy({ oizys_output_failure($0.output) == nil }) else { return }
        if oizys_driver_service_control(driver) < 0 {
            for output in capture.outputs.values { oizys_output_fail(output.output, "control session keepalive failed") }
            return
        }
...
func captureFrames(_ raw: UnsafeRawPointer?, _ head: Int32) -> Int32 {
    ...
    return oizys_output_frames(output.output)
}
@_cdecl("oizys_capture_failure")
func captureFailure(_ raw: UnsafeRawPointer?, _ head: Int32) -> UnsafePointer<CChar>? {
    ...
    return oizys_output_failure(output.output)
}
@_cdecl("oizys_capture_head_count")
func captureCount(_ raw: UnsafeRawPointer?) -> Int32 { raw.map { Int32(instance($0).count) } ?? 0 }
@_cdecl("oizys_capture_profile_report")
func captureReport(_ raw: UnsafeMutableRawPointer?, _ title: UnsafePointer<CChar>?) {
#if !OIZYS_PRODUCTION
    guard let raw else { return }
    let capture = instance(raw)
    capture.worker.sync {
        if oizys_profile_active != 0 { oizys_profile_report(title) }
        else if let title { puts(title) }
        let now = ProcessInfo.processInfo.systemUptime
        let elapsed = now - capture.lastReport
        for head in 0..<capture.count {
            guard let output = capture.outputs[head] else { continue }
            let frames = oizys_output_frames(output.output)
            let delta = max(0, Int64(frames) - Int64(capture.previousFrames[head] ?? 0))
            if elapsed > 0, let display = capture.displayIDs[head] {
                print("OIZYS_METRIC head=\(head) display=\(display) capture_fps=\(Double(delta) / elapsed) interval=\(elapsed)")
            }
            capture.previousFrames[head] = frames
            oizys_output_report(output.output)
        }
        capture.lastReport = now
        if oizys_profile_active != 0 { oizys_profile_reset() }
    }
#endif
}
@_cdecl("oizys_capture_stop")
