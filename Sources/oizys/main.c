...
#define OIZYS_HEADS 2
#define OIZYS_HEAD_W 1920
#define OIZYS_HEAD_H 1080
...
static void ensure_logs_dir(void) {
    if (OIZYS_DIAGNOSTICS) mkdir("logs", 0700);
}
...
static void print_usage(void) {
    puts("oizys tui                  interactive monitor/settings controls (also the terminal default)\n"
    ...
    if (!OIZYS_DIAGNOSTICS) {
        puts("oizys build-info | probe | routes\n"
             "oizys config list | get <key> | set <key> <value> | reset\n"
             "oizys ddc list | caps | dump | get <code> | set <code> <value>\n"
             "oizys serve --takeover     foreground service (Ctrl-C stops it)\n"
             "Production has no developer GUI or test commands.");
        return;
    }
    puts("oizys — open-source DisplayLink Ridge driver (C)\n\n"
         ...
         "  oizys verify --takeover [--seconds N]  measured pattern proof -> logs/verify.json\n"
         "  oizys bench              encoder throughput on a synthetic surface\n"
         "  oizys profile            per-zone profile of a scanout sequence\n"
         "  oizys run --takeover [--stats | --profile]  forward two encoded desktops\n"
         "                           --stats: capture cadence/age; --profile: fine codec timing\n"
         "  oizys serve --takeover [--stats]  restart Oizys after faults or reconnects\n"
         "  oizys confirm            you looked at both panels and they were right\n\n"
         "  oizys config list | get <key> | set <key> <value> | reset | selftest\n"
         "  oizys ddc list | caps | dump | get <code> | set <code> <value>\n"
         "                           monitor menu over DDC/CI (needs a native display pipe)\n\n"
         "  overall_pass in logs/verify.json is only ever set by `oizys confirm`,\n"
         ...
         "  oizys stop-dlm | start-dlm");
}

static void cmd_probe(void) {
    OizysHub *hubs = calloc(OIZYS_MAX_HUBS, sizeof(*hubs));
    if (!hubs) {
        return;
    }
    int n = oizys_usb_probe(hubs, OIZYS_MAX_HUBS);
    if (n == 0) {
...
static OizysVirtualDisplay *make_head(const char *name, uint32_t serial) {
    ...
    return oizys_virtual_display_create(&desc);
}
...
static int active_heads(int *out) {
    int count = 0;
    for (int head = 0; head < OIZYS_HEADS; head++) {
        if (oizys_config_head_active(head)) {
            out[count++] = head;
        }
    }
    return count;
...
static const char *HEAD_NAME[OIZYS_HEADS] = {"Oizys Left", "Oizys Right"};
static const uint32_t HEAD_SERIAL[OIZYS_HEADS] = {0x4d560001, 0x4d560002};
static const char *SPARE_NAME[OIZYS_HEADS] = {"Oizys Spare A", "Oizys Spare B"};
static const uint32_t SPARE_SERIAL[OIZYS_HEADS] = {0xdead0001, 0xdead0002};
...
static int check_native_route(void) {
    ...
    if (!oizys_ddc_native_display_id()) {
        fputs("cannot verify the native display requested by heads.native; leaving DisplayLink "
              ...
              "already exist. Run `oizys routes` to inspect the current setup.\n", stderr);
        return 0;
    }
    return 1;
}
...
static int cmd_routes(void) {
    const OizysConfig *config = oizys_config();
    for (int head = 0; head < OIZYS_HEADS; head++) {
        printf("%s: %s\n", HEAD_NAME[head], oizys_config_head_active(head)
...
static void destroy_heads(OizysVirtualDisplay **heads) {
    for (int head = 0; head < OIZYS_HEADS; head++) {
        oizys_virtual_display_destroy(heads[head]);
        heads[head] = NULL;
    }
}

static int create_heads(OizysVirtualDisplay **heads, const int *active, int count,
...
static int any_head_unit_shared(OizysVirtualDisplay **heads, const int *active, int count) {
    for (int i = 0; i < count; i++) {
        if (oizys_display_unit_is_shared(oizys_virtual_display_id(heads[active[i]]))) {
            return 1;
        }
    }
    return 0;
...
static int make_heads(OizysVirtualDisplay **heads) {
    int active[OIZYS_HEADS];
     ...
     */
    destroy_heads(heads);
    OizysVirtualDisplay *decoys[OIZYS_HEADS] = {NULL, NULL};
    ...
    (void)create_heads(decoys, all, OIZYS_HEADS, SPARE_NAME, SPARE_SERIAL);
    sleep_seconds(0.5);
...
static void report_sidecar_displays(void) {
    ...
    for (uint32_t i = 0; i < count; i++) {
        if (oizys_display_is_sidecar(ids[i])) {
...
static void report_head_modes(const uint32_t *ids, int count) {
    for (int head = 0; head < count; head++) {
        ...
        if (oizys_display_mode(ids[head], &width, &height, &hz) == 0) {
...
static int head_display_ids(OizysVirtualDisplay **heads, uint32_t *ids) {
    int count = 0;
    for (int head = 0; head < OIZYS_HEADS; head++) {
        ids[head] = heads[head] ? oizys_virtual_display_id(heads[head]) : 0;
        count += ids[head] != 0;
    }
    return count;
}
...
static void cmd_displays(void) {
    ...
    static OizysVirtualDisplay *heads[OIZYS_HEADS];
...
static void cmd_diagnose(int takeover) {
    if (!takeover) {
        ...
        puts("re-run: oizys diagnose --takeover");
        return;
    }
    ensure_logs_dir();
    oizys_log_open("logs/native-diagnose.log");
    puts("stopping DisplayLink Manager for a control-only native diagnostic…");
    oizys_stop_displaylink();
    sleep_seconds(0.6);
    OizysSession *session = oizys_session_open(1);
    if (!session) {
        fputs("failed to claim the DisplayLink interface\n", stderr);
        oizys_start_displaylink();
        return;
    }
    OizysDriver *driver = oizys_driver_engage(session, 0x6000);
    if (!driver) {
        fputs("native encrypted-session engagement failed; see logs/native-diagnose.log\n", stderr);
        oizys_session_close(session);
        oizys_start_displaylink();
        return;
    }
    int fetch_result = oizys_driver_fetch_edids(driver);
    for (int head = 0; head < OIZYS_HEADS; head++) {
        OizysHeadStatus status;
        if (oizys_driver_get_head(driver, (uint8_t)head, &status) != 0) {
            continue;
        }
 ...
 */
static int write_verification(OizysSession *session, OizysDriver *driver,
                              const char *expected_output, const int *capture_frames,
                              int capture_head_count, int human_confirmed) {
    uint8_t identity_bytes[64];
    int identity_length =
        oizys_session_get_identity(session, identity_bytes, (int)sizeof(identity_bytes));
    OizysIdentity identity;
    memset(&identity, 0, sizeof(identity));
    int identity_valid =
        identity_length > 0 && oizys_identity_parse(&identity, identity_bytes, identity_length) == 0;
    char firmware[32] = "";
    if (identity_valid) {
    ...
    const char *owner = "";
    if (hubs) {
        int hub_count = oizys_usb_probe(hubs, OIZYS_MAX_HUBS);
    ...
    int owner_pass = strcasestr(owner, "oizys") != NULL;

    OizysDriverVerification native;
    memset(&native, 0, sizeof(native));
    int native_valid = oizys_driver_get_verification(driver, &native) == 0;
    int ake_pass =
    ...
    OizysHeadStatus heads[OIZYS_HEADS];
    int head_valid[OIZYS_HEADS];
    int head_pass[OIZYS_HEADS];
    ...
    for (int head = 0; head < OIZYS_HEADS; head++) {
        ...
        driven_heads++;
        head_valid[head] = oizys_driver_get_head(driver, (uint8_t)head, &heads[head]) == 0;
    ...
    int capture_pass = 1;
    if (capture_frames) {
        for (int head = 0; head < capture_head_count && head < OIZYS_HEADS; head++) {
            if (oizys_config_head_active(head)) {
    ...
    for (int head = 0; head < OIZYS_HEADS; head++) {
        ...
        fprintf(file, ", \"driven_over_dock\": %s, \"pass\": %s}%s\n",
                json_bool(oizys_config_head_active(head)), json_bool(head_pass[head]),
                head + 1 < OIZYS_HEADS ? "," : "");
    }
    fputs("  ],\n", file);
...
static int cmd_confirm(void) {
    ...
    if (!file) {
        fputs("no logs/verify.json; run `oizys run --takeover` or `oizys verify` first\n", stderr);
        return 0;
    }
    char text[8192];
...
static int cmd_patterns(int takeover, int seconds, int confirmed) {
    if (!takeover) {
        puts("refusing to claim USB while DisplayLink Manager may own it");
        puts("re-run: oizys patterns --takeover --seconds 90");
        return 0;
    }
    if (!check_native_route()) return 0;
    ensure_logs_dir();
    oizys_log_open("logs/native-patterns.log");
    ...
    while (time(NULL) < deadline) {
        uint8_t colour[OIZYS_HEADS][3] = {{(uint8_t)(phase ? 0 : 255), 0, (uint8_t)(phase ? 255 : 0)},
                                          {(uint8_t)(phase ? 255 : 0), 255, 0}};
        for (int head = 0; head < OIZYS_HEADS; head++) {
            if (!oizys_config_head_active(head)) {
                continue;
            }
            if (oizys_driver_present_solid(driver, (uint8_t)head, colour[head][0], colour[head][1],
                                           colour[head][2]) < 0) {
...
static struct {
    OizysCapture *capture;
    OizysVirtualDisplay *heads[OIZYS_HEADS];
    OizysDriver *driver;
...
static void run_shutdown(const char *reason, int code) {
    ...
    puts(reason);
    if (g_run.capture) {
        oizys_capture_stop(g_run.capture);
        g_run.capture = NULL;
    }
    destroy_heads(g_run.heads);
    if (g_run.driver) {
        oizys_driver_destroy(g_run.driver);
        g_run.driver = NULL;
    }
    if (g_run.session) {
        oizys_session_close(g_run.session);
        g_run.session = NULL;
    }
    if (g_run.supervisor_fd < 0) {
        ...
        puts("released hub; Oizys supervisor owns recovery");
    }
    exit(code);
}
...
static int cmd_run(int takeover, int profile, int stats, int supervisor_fd) {
    if (!takeover) {
        puts("refusing to claim USB while DisplayLink Manager may own it");
        puts("re-run: oizys run --takeover");
        return 0;
    }
    if (!check_native_route()) return 0;
    ...
    g_run.session = oizys_session_open(1);
    if (!g_run.session) {
        fputs("failed to claim IOUSBHostInterface ff/00/03\n", stderr);
        if (supervisor_fd < 0) oizys_start_displaylink();
        return 0;
    }
    g_run.driver = oizys_driver_engage(g_run.session, 0x6000);
    if (!g_run.driver) {
     ...
     * capture stream and the video endpoint off it -- oizys_driver_present_bgra_mosaic and
     * oizys_driver_refresh_head both refuse a head that was never armed.
     */
    for (int head = 0; head < OIZYS_HEADS; head++) {
        if (!oizys_config_head_active(head)) {
        ...
        if (oizys_driver_fetch_edid(g_run.driver, (uint8_t)head) != 0 ||
            oizys_driver_activate_1080p60(g_run.driver, (uint8_t)head) != 0) {
    ...
    if (make_heads(g_run.heads) != 0) {
        fputs("could not create the Oizys CGVirtualDisplays on unshared display units\n", stderr);
        run_shutdown("aborting", 1);
    }
    uint32_t ids[OIZYS_HEADS];
    ...
    for (int attempt = 0; attempt < 5; attempt++) {
        int mirrored = 0;
        for (int head = 0; head < OIZYS_HEADS; head++) {
            mirrored = mirrored || (ids[head] && oizys_display_is_mirrored(ids[head]));
        }
        if (!mirrored) {
            break;
        }
    ...
    for (int head = 0; head < OIZYS_HEADS; head++) {
        ...
        if (ids[head] && oizys_display_is_mirrored(ids[head])) {
            fprintf(stderr, "head %d is still in a mirror set; turn mirroring off for it\n", head);
            run_shutdown("aborting", 1);
        }
    }
    uint32_t packed[OIZYS_HEADS];
    oizys_displays_watch(packed, packed_display_ids(ids, packed), OIZYS_HEAD_W, OIZYS_HEAD_H);
    report_sidecar_displays();

    puts("starting ScreenCaptureKit desktop forwarding…");
    if (profile) {
        oizys_profile_reset();
        oizys_profile_enable(1);
    }
    char capture_error[256] = "";
    g_run.capture =
        oizys_capture_start(ids, OIZYS_HEADS, g_run.driver, capture_error, sizeof(capture_error));
    if (!g_run.capture) {
        fprintf(stderr, "ScreenCaptureKit start failed: %s\n", capture_error);
        fputs("grant Screen Recording access to oizys, then retry\n", stderr);
        run_shutdown("aborting", 1);
    }
    for (int waited = 0; waited < 50; waited++) {
        int ready = 1;
        for (int head = 0; head < OIZYS_HEADS; head++) {
            ready = ready && (!ids[head] || oizys_capture_frames(g_run.capture, head) > 0);
        }
        if (ready) {
            break;
        }
    ...
    int frames[OIZYS_HEADS];
    for (int head = 0; head < OIZYS_HEADS; head++) {
        frames[head] = ids[head] ? oizys_capture_frames(g_run.capture, head) : 0;
        if (ids[head] && frames[head] == 0) {
            fprintf(stderr, "ScreenCaptureKit delivered no initial frame for head %d\n", head);
            run_shutdown("aborting", 1);
        }
    }
    oizys_capture_start_refresh_clock(g_run.capture, g_run.driver, OIZYS_HEADS,
                                      oizys_config()->refresh_clock_hz);
    char summary[160] = "";
    for (int head = 0; head < OIZYS_HEADS; head++) {
        if (ids[head]) {
    ...
    if (OIZYS_DIAGNOSTICS) write_verification(g_run.session, g_run.driver, summary, frames, OIZYS_HEADS, 0);
    puts("");
    if (driven == OIZYS_HEADS) {
    ...
    puts(supervisor_fd < 0 ? "Ctrl-C restores DisplayLink Manager. If the panels were right, run:"
                          : "Oizys owns recovery. Ctrl-C stops the service. To confirm the panels, run:");
    puts("  build/Release/oizys confirm");
    ...
    dispatch_source_set_event_handler(watchdog, ^{
      for (int head = 0; head < OIZYS_HEADS; head++) {
          if (oizys_capture_failure(g_run.capture, head)) {
              char reason[96];
      ...
      if (g_run.supervisor_fd >= 0 && write(g_run.supervisor_fd, ".", 1) < 0 &&
          ...
          run_shutdown("Oizys supervisor disappeared", 1);
      }
    });
    ...
    if (profile || stats) {
        ...
        dispatch_source_set_event_handler(reporter, ^{
          static int window;
          char title[96];
          int frames0 = oizys_capture_frames(g_run.capture, 0);
          int frames1 = oizys_capture_frames(g_run.capture, 1);
          snprintf(title, sizeof(title), "5 s window %d — capture frames %d / %d", ++window,
                   frames0, frames1);
          oizys_capture_profile_report(g_run.capture, title);
        });
...
static int cmd_config(int argc, char **argv) {
    ...
    if (strcmp(action, "get") == 0 && argc > 1) {
        char value[64];
        if (oizys_config_get(argv[1], value, sizeof(value)) != 0) {
    ...
    if (strcmp(action, "set") == 0 && argc > 2) {
        int rc = oizys_config_set(argv[1], argv[2]);
        if (rc == -1) {
            fprintf(stderr, "unknown key %s\n", argv[1]);
            return 0;
        }
        ...
        if (rc != 0) {
            fputs("Could not persist the setting.\n", stderr);
            return 0;
        }
        char stored[64];
        oizys_config_get(argv[1], stored, sizeof(stored));
        printf("%s = %s\n", argv[1], stored);
        return 1;
    }
    fputs("oizys config list | get <key> | set <key> <value> | reset | selftest\n", stderr);
    return 0;
}
...
static int cmd_serve(int takeover, int profile, int stats) {
    if (!takeover) {
        fputs("service takeover requires `oizys serve --takeover`\n", stderr);
        return 0;
    }
    if (!check_native_route()) return 0;
    int active[OIZYS_HEADS];
    ...
    return oizys_supervise(resolved, profile, stats);
}
...
static int cmd_ddc(int argc, char **argv) {
    ...
    for (int i = 0; i < argc; i++) {
        if (strcmp(argv[i], "--display")) continue;
        char *end = NULL;
        errno = 0;
        unsigned long value = i + 1 < argc ? strtoul(argv[i + 1], &end, 0) : 0;
        if (errno || !value || value > UINT32_MAX || !end || *end ||
            !CGDisplayIsOnline((uint32_t)value)) {
            fputs("--display requires an online display ID from `oizys monitors`.\n", stderr); return 0;
        }
    ...
    uint32_t id = requested ? requested : oizys_ddc_native_display_id();
    if (!id) {
        fputs("no display has an I2C path; run `oizys ddc list`\n", stderr);
        return 0;
    }
    OizysDDCDisplay *display = oizys_ddc_open(id);
    if (!display) {
    ...
    if (strcmp(action, "caps") == 0) {
        char caps[2048];
        if (oizys_ddc_capabilities(display, caps, sizeof(caps)) == 0 && caps[0]) {
        ...
        } else if (oizys_ddc_get_vcp(display, (uint8_t)code, &current, &maximum) == 0) {
            const char *name = oizys_ddc_vcp_name((uint8_t)code);
        ...
        char *end = NULL;
        errno = 0;
        long value = strtol(argv[2], &end, 0);
        if (code < 0 || errno || end == argv[2] || *end || value < 0 || value > 0xffff) {
            fputs("usage: oizys ddc set <code> <value>\n", stderr);
        } else if (oizys_ddc_set_vcp(display, (uint8_t)code, (uint16_t)value) == 0) {
            uint16_t current = 0, maximum = 0;
            if (oizys_ddc_get_vcp(display, (uint8_t)code, &current, &maximum) == 0) {
        ...
        fputs("oizys ddc list | caps | dump | get <code> | set <code> <value>\n", stderr);
    }
    oizys_ddc_close(display);
    return ok;
}

int main(int argc, char **argv) {
    setvbuf(stdout, NULL, _IOLBF, 0);
    int terminal_result = oizys_terminal_command(argc, argv);
    if (terminal_result >= 0) return terminal_result;
    ...
    if (strcmp(command, "build-info") == 0) {
        printf("{\"product\":\"Oizys\",\"diagnostics\":%s,\"verbose\":%s,\"displaylink_fallback\":%s}\n",
               OIZYS_DIAGNOSTICS ? "true" : "false", OIZYS_VERBOSE ? "true" : "false",
               OIZYS_ALLOW_DISPLAYLINK ? "true" : "false");
        return 0;
    }
    if (!OIZYS_DIAGNOSTICS && (has_flag(rest_argc, rest, "--stats") ||
        ...
        strcmp(command, "displays") == 0 ||
        (strcmp(command, "config") == 0 && rest_argc > 0 && strcmp(rest[0], "selftest") == 0))) {
        fputs("This Oizys build excludes developer diagnostics. Use a debug build.\n", stderr);
        return 2;
    }
    if (!OIZYS_ALLOW_DISPLAYLINK && strcmp(command, "start-dlm") == 0) {
        fputs("DisplayLink fallback is excluded from this Oizys build.\n", stderr); return 2;
    }
    ...
    if (strcmp(command, "probe") == 0) {
        ...
        int lease = worker_fd < 0 ? oizys_lock_driver() : -1;
        if (worker_fd < 0 && lease < 0) {
            fputs("Another Oizys driver is running. Stop it with `oizys service stop`.\n", stderr);
            return 1;
        }
        int result = cmd_run(takeover, has_flag(rest_argc, rest, "--profile"),
        ...
        return oizys_bench_encoder();
    } else if (strcmp(command, "profile") == 0) {
        return oizys_profile_encoder();
