...
#define STOP_SECONDS (OIZYS_DIAGNOSTICS ? 10 : 3)
...
int oizys_lock_driver(void) {
    ...
    snprintf(path, sizeof(path), "%soizys-driver.lock", directory);
...
static int supported_dock_present(void) {
    OizysHub hubs[OIZYS_MAX_HUBS];
    int count = oizys_usb_probe(hubs, OIZYS_MAX_HUBS);
    // The transport currently selects one device. Never seize an ambiguous set.
    return count == 1 && hubs[0].pid == 0x6000 && oizys_hub_is_dl3(&hubs[0]);
}
...
static pid_t start_worker(const char *executable, int profile, int stats, int *status_fd) {
    ...
    close(channel[1]);
    if (error) {
        fprintf(stderr, "could not start Oizys worker: %s\n", strerror(error));
        close(channel[0]);
        return -1;
    }
    *status_fd = channel[0];
...
static void stop_worker(pid_t child) {
    ...
    fprintf(stderr, "worker did not stop in %d seconds; terminating it\n", STOP_SECONDS);
    kill(child, SIGKILL);
    while (waitpid(child, NULL, 0) < 0 && errno == EINTR) {}
}

int oizys_supervise(const char *executable, int profile, int stats) {
    int lock = oizys_lock_driver();
    if (lock < 0) {
        fputs("another Oizys service is running, or its lock could not be acquired\n", stderr);
        return 0;
    }
    ...
    int restore_vendor = OIZYS_ALLOW_DISPLAYLINK && displaylink_running();
    puts("Oizys service started; failures will restart Oizys, not DisplayLink");
    int backoff = 1, waiting = 0, ok = 1, failures = 0, claimed = 0;
    while (!stopping) {
        if (!supported_dock_present()) {
            if (!waiting) puts("waiting for one supported Ridge dock; Ctrl-C stops Oizys");
            waiting = 1;
        ...
        while (!stopping) {
            ...
            if (read(status_fd, status, sizeof(status)) > 0) {
                ...
                if (!ready) {
                    ready = now;
                    printf("Oizys worker ready in %.1fs\n", now - began);
                }
            ...
            if (result == child || (result < 0 && errno == ECHILD)) {
                reaped = 1;
                puts("Oizys worker exited; releasing its session before retry");
                break;
            }
            if (monotonic_seconds() >= deadline) {
                fputs("Oizys worker stopped responding; restarting its session\n", stderr);
                break;
            }
            if (monotonic_seconds() >= topology_check) {
                if (!supported_dock_present()) break;
                topology_check = monotonic_seconds() + 1;
            }
            pause_briefly();
        }
        ...
        failures++;
        if (OIZYS_ALLOW_DISPLAYLINK && restore_vendor && failures >= 3) {
            fputs("Oizys failed three recovery attempts; handing back to DisplayLink\n", stderr);
            ok = 0; break;
        }
        printf("retrying Oizys in %ds\n", backoff);
        double retry = monotonic_seconds() + backoff;
        while (!stopping && monotonic_seconds() < retry) pause_briefly();
        if (backoff < 8) backoff *= 2;
    }
    if (restore_vendor && claimed) {
        oizys_start_displaylink();
        puts("Oizys stopped; restored the DisplayLink session that preceded takeover");
    } else {
        puts("Oizys stopped; DisplayLink was not running and was not launched");
    }
