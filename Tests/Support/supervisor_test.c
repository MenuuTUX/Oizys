...
static size_t test_confstr(int name, char *buffer, size_t capacity) {
    (void)name;
    const char *directory = getenv("OIZYS_TEST_DIR");
...
static int test_proc_listallpids(void *buffer, int size) {
    if (!getenv("OIZYS_TEST_VENDOR")) return 0;
...
#include "../../Sources/oizys/supervisor.c"

int oizys_usb_probe(OizysHub *hubs, int capacity) {
    (void)capacity;
    FILE *file = fopen(getenv("OIZYS_TEST_TOPOLOGY"), "r");
    int count = 0;
...
int oizys_hub_is_dl3(const OizysHub *hub) { (void)hub; return 1; }
int oizys_stop_displaylink(void) { puts("TEST vendor stop"); return 0; }
int oizys_start_displaylink(void) { puts("TEST vendor restore"); return 0; }
int main(int argc, char **argv) {
    if (argc != 2) return 2;
    setvbuf(stdout, NULL, _IOLBF, 0);
    return oizys_supervise(argv[1], 0, 0) ? 0 : 1;
}
